...
GameStateLose::GameStateLose(Graphics* graphics, ObjectShader* shaderManager, Target_State* pState) {
	mGraphics = graphics;
	p_state = pState;
...
int GameStateLose::Update(float deltaTime, int score) {
	ImGuiIO& io = ImGui::GetIO();
	...
	ImGui::GetWindowDrawList()->AddText(buttonPos, IM_COL32(255, 255, 255, 255), "Again", nullptr);
	// Check for button click
	if (ImGui::IsMouseReleased(0) && ImGui::IsMouseHoveringRect(buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y))) {
		ImGui::End();
		ImGui::Render();
	...
	textPos = ImVec2((window_size.x - ImGui::CalcTextSize("Score: 100000").x) * 0.5f, (window_size.y - buttonSize.y));
	ImGui::SetCursorPos(textPos);
