...
GameStateMainMenu::GameStateMainMenu(Graphics* graphics, ObjectShader* shaderManager, Target_State* pState) {
	mGraphics = graphics;
	p_state = pState;
...
int GameStateMainMenu::Update(float deltaTime) {
	ImGuiIO& io = ImGui::GetIO();
	...
	ImGui::GetWindowDrawList()->AddText(buttonPos, IM_COL32(255, 255, 255, 255), "Easy", nullptr);
	...
	ImGui::GetWindowDrawList()->AddText(buttonPos, IM_COL32(255, 255, 255, 255), "Normal", nullptr);
	// Check for button click
	if (ImGui::IsMouseReleased(0) && ImGui::IsMouseHoveringRect(buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y))) {
		ImGui::End();
		ImGui::Render();
	...
	//hard button
	buttonPos = ImVec2((window_size.x - buttonSize.x) * 0.5f, (window_size.y + (buttonSize.y * 1.5f)) * 0.5f);
	// Draw the play button
	if (ImGui::IsMouseHoveringRect(buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y))) {
		ImGui::GetWindowDrawList()->AddRectFilled(buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y), IM_COL32(0, 70, 200, 255));
	}
	...
	ImGui::GetWindowDrawList()->AddText(buttonPos, IM_COL32(255, 255, 255, 255), "Hard", nullptr);
	// Check for button click
	if (ImGui::IsMouseReleased(0) && ImGui::IsMouseHoveringRect(buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y))) {
		ImGui::End();
		ImGui::Render();
